...
log.add_argument('--log-dir',type=str,default='/home/pikey/Data/II/lwgan_gp_base_vae_wgan_gp_g16',help='log directory')

model.add_argument('--z-dim',type=int,default=1024,metavar='dz',help='latent_space_dimension')
model.add_argument('--g-dim',type=int,default=16,metavar='dz',help='latent_space_dimension')
...
AE = vae_wgan_gp.VAE(args.z_dim)
AE.load_state_dict(torch.load('/home/pikey/Data/II/vae_wgan_gp/model/vae/epoch399.pkl'))
AE.to(gpu)
AE.eval()

AE_Dis = vae_wgan_gp.Discriminator()
AE_Dis.load_state_dict(torch.load('/home/pikey/Data/II/vae_wgan_gp/model/di/epoch399.pkl'))
# utils.freeze(AE)
m = lwgan_gp.LWGAN_GP(args.g_dim,args.z_dim).to(gpu)
...
#todo ----------------------------------set title-----------------------------------
setproctitle.setproctitle('lwgan_gp:{}'.format(args.gpu))

#todo"""--------------------------------train------------------------------------"""
def train():
	print('train begin')
	noise= utils.decay_scale(1e-2,1e-5,100000)
	# train
	step = 0
	for epoch in trange(args.epoch):
		...
		for true, flaw, region in dldr:
			...
			#todo"""--------------------------------train------------------------------------"""
			gen_loss = lwgan_gp.generator_loss(judge_fake)
			dis_loss = lwgan_gp.discriminator_loss(judge_real,judge_fake)
			dis_loss+=lwgan_gp.gradient_penalty(m.discriminator,AE.z,fake_f)
