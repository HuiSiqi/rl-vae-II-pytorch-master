...
train.add_argument('--bs',type=int,default=80,metavar='BS',help='training batch size')
train.add_argument('--lr',type=float,default=1e-3,metavar='lr',help='learning rate')

log.add_argument('--log-dir',type=str,default='/home/pikey/Data/II/lwgan',help='log directory')

model.add_argument('--z-dim',type=int,default=100,metavar='dz',help='latent_space_dimension')
...
AE = ae.AE(args.z_dim)
AE.load_state_dict(torch.load('/home/pikey/Data/II/ae/model/epoch399.pkl'))
AE.to(gpu)
AE.eval()
m = lwgan.LWGAN(args.z_dim).to(gpu)
...
def train():
	print('train begin')
	noise= utils.decay_scale(1e-2,0,10000)
	# train
	step = 0
	for epoch in trange(args.epoch):
		...
		for true, flaw, region in dldr:
			...
			seed =  torch.randn(args.bs,16).to(gpu)

			#todo get the fake judge
			fake_f = m.gen(seed)
			fake_img = AE.dec(fake_f)
