...
env.add_argument('--lgan',type=str,default='lwgan_gp',help='the lgan model')
env.add_argument('--ae',type=str,default='vae_wgan_gp',help='the autoencoder model')
env.add_argument('--wrec',type=float,default=1,help='the weight of reconstruction reward')
...
env.add_argument('--dim_a',type=int,default=16,help='action dim')
env.add_argument('--dim_s',type=int,default=16,help='state dim')
...
model.add_argument('--h2_dim',type=int,default=128,help="h2 dimension")
model.add_argument('--sigma',type=float,default=0.01,help="action noise variance")
model.add_argument('--gamma',type=float,default=0.2,help="reward decay rate")
model.add_argument('--max_a',type=float,default=5.,help="max_action")
...
memo.add_argument('--wmup',type=int,default=10,help="data number before to train ")
...
print('lgan:{}      ae:{}       bs:{}       wrec,wl2,wconsis:{},{},{}'.format(args.lgan,args.ae,args.dst_bs,args.wrec,args.wl2,args.wconsis))
print('--------------------------------------------')
...
def warm_up():
	print('---------------------------warm up-----------------------------')
	for i in tqdm.trange(args.wmup):
		e.reset()
		s = e.observe()
		a = player.actor_target(torch.Tensor(s).to(gpu))
		a += (torch.rand_like(a)*args.sigma).to(gpu)
		s,a,r,s_,t = e.step(a)
		player.store(s,a,r,s_,t)
...
def explore(n):
	for i in range(n):
		...
		s, a_, r, s_, t = e.step(a)
		player.store(s, a_, r, s_, t)
		print('reward:{}    action_norm:{}'.format(r.mean(),a.norm(2,dim=1,keepdim=True)[0]))
		# print('action:{}'.format(a[0][:16]))
