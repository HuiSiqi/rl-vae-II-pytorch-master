...
from train.ddpg.conv.one_step.E34 import tool
...
train.add_argument('--epoch',type=int,default=30,metavar='TS',help='training steps')
train.add_argument('--bs',type=int,default=48,metavar='BS',help='training batch size')
...
log.add_argument('--log-dir',type=str,default='/home/pikey/Data/II/34/ae',help='log directory')
...
def train():
	...
	for epoch in trange(args.epoch):
		bt = 0
		for true, flaw, mask,patch,region in dldr:
			bt += 1
			step += 1
			#todo"""--------------------------------forward------------------------------------"""
			pos = torch.cat([true,torch.zeros_like(mask)],dim=1)
			neg = torch.cat([flaw,mask],dim=1)
			input=  torch.cat([pos,neg],dim=0).to(gpu)
			rec,loss= m(input)
		...
		if (epoch+1)%5==0:
			...
			torch.save(m.module.cpu().state_dict(), os.path.join(args.log_dir, 'model', 'epoch{}.pkl'.format(epoch)))
			m.module.to(gpu)
